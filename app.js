...
function buildListRow(c, i) {
  ...
  return `
<div class="list-row${isExp?" expanded":""}" style="animation-delay:${i*0.03}s" id="row-${c.id}">
  ...
  <div class="lr-update">
    <span class="status-badge ${sbClass}" style="font-size:10px;margin-bottom:4px;display:inline-flex">${c.status}</span>
    <div>${esc(c.update||"-")}</div>
  </div>
  <div class="lr-dept">${c.department?`<span class="dept-badge dept-${deptClass}">${esc(c.department)}</span>`:`<span style="color:var(--text3)">—</span>`}</div>
  <div class="lr-due">${formatDueDate(c.duedate)}</div>
  <div class="lr-dep">${deps.length?deps.map(d=>`<span class="dep-badge">${esc(d)}</span>`).join(""):`<span style="color:var(--text3)">—</span>`}</div>
  <div class="lr-who"><div class="avatar">${initials}</div><span class="who-name">${esc(c.updatedBy||"?")}</span></div>
