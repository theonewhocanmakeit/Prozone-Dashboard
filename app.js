// ProZone Realty Dashboard — Final v20
const COLLECTION = "project_updates";
const ICONS = {"Bandra Residences":"🏙️","Worli Skyline":"🌆","Andheri Heights":"🏗️","Juhu Marina":"🌊","Powai Greens":"🌿"};
const AV_COLORS = ["#6c47ff","#10b981","#ef4444","#f59e0b","#db2777","#7c3aed","#0891b2","#ea580c"];

let allDocs=[], activeProject="all", activeSubProject="all", activeStatus="all";
let activeDept="all", activeSort="newest", activeDateFilter="all";
let currentSearch="", currentView="list", pendingDeps=[], pendingImages=[], expandedRows=new Set();
...
function buildSidenavProjects() {
  ...
  let html = `<button class="sidenav-item${activeProject==="all"?" active":""}" onclick="selectProject('all',this);closeSidebar()">
    <span class="proj-dot" style="background:#7c6ef7"></span>All Projects</button>`;
  projects.forEach((p,i) => {
    html += `<div class="proj-item-wrap">
      <button class="sidenav-item proj-item${activeProject===p?" active":""}" onclick="selectProject('${esc(p)}',this);closeSidebar()">
        <span class="proj-dot" style="background:${colors[i%colors.length]}"></span>${esc(p)}
      </button>
      <button class="proj-delete-btn" title="Delete all updates for ${esc(p)}" onclick="confirmDeleteProject('${esc(p)}')">🗑️</button>
    </div>`;
  });
...
function filterDept(d, btn) {
  activeDept = d;
  document.querySelectorAll(".dept-chip").forEach(b=>b.classList.remove("active"));
  if(btn) btn.classList.add("active");
  renderView();
}
...
function setMobActive(btn){document.querySelectorAll(".mob-btn").forEach(b=>b.classList.remove("active"));if(btn)btn.classList.add("active");}
...
// ── RENDER LIST ──
function renderView(){if(currentView==="list")renderList();else renderBoard();}
...
function renderList() {
  ...
  expandedRows.forEach(id=>{const el=document.getElementById("expand-"+id);if(el)el.style.display="block";});
}
...
function buildListRow(c, i) {
  ...
  const avColor=AV_COLORS[c.updatedBy?c.updatedBy.charCodeAt(0)%AV_COLORS.length:0];
...
<div class="list-row${isExp?" expanded":""}" style="animation-delay:${i*0.04}s" id="row-${c.id}">
  ...
  <div class="lr-who-cell">
    ...
    <div class="who-info"><span class="who-name">${esc(c.updatedBy||"?")}</span></div>
  </div>
  ...
  <div class="row-actions">
    ...
    <button class="action-btn delete-btn" onclick="confirmDelete('${c.id}','${esc(c.project)}')">🗑️ Delete</button>
...
<div class="list-row-expand" id="expand-${c.id}" style="display:${isExp?"block":"none"}">
  ...
  ${c.history.length>1?`<div class="expand-title">Update History</div><div class="hist-entries">${c.history.slice(1,6).map(h=>`<div class="hist-entry"><span class="status-badge ${h.status==="On Track"?"sb-ontrack":h.status==="Delayed"?"sb-delayed":"sb-completed"}" style="font-size:10px;margin-bottom:4px;display:inline-flex">${h.status}</span><div class="hist-entry-note">${esc(h.update||"")}</div><div class="hist-entry-meta">by ${esc(h.updatedBy||"?")} · ${getTimeAgo(h.timestamp)}</div></div>`).join("")}</div>`:""}
</div>`;
}
...
function renderBoard(){
  ...
  let cards=Object.values(keyMap);
  if(activeProject!=="all")cards=cards.filter(c=>c.project===activeProject);
  if(activeSubProject!=="all")cards=cards.filter(c=>(c.subproject||"")===activeSubProject);
  if(activeDept!=="all")cards=cards.filter(c=>c.department===activeDept);
  if(currentSearch.trim()){const q=currentSearch.toLowerCase();cards=cards.filter(c=>(c.project||"").toLowerCase().includes(q));}
  const ontrack=cards.filter(c=>c.status==="On Track");
  const delayed=cards.filter(c=>c.status==="Delayed");
...
function buildBoardCard(c){
  ...
  return `<div class="board-card">
    ...
    <div class="bc-meta">
      <div class="bc-who"><div class="av" style="background:${avColor};width:18px;height:18px;font-size:8px">${initials}</div>${esc(c.updatedBy||"?")}</div>
      <div style="display:flex;align-items:center;gap:6px">
        ...
        <button class="bc-edit" style="color:var(--red);border-color:var(--red-bd)" onclick="confirmDelete('${c.id}','${esc(c.project)}')">🗑️</button>
      </div>
    </div>
  </div>`;
}
...
function openModal(){
  ...
  setTimeout(()=>document.getElementById("f-project").focus(),200);
}
...
function toggleMic(){
  const btn=document.getElementById("mic-btn"),status=document.getElementById("mic-status");
  if(!("webkitSpeechRecognition" in window)&&!("SpeechRecognition" in window)){showToast("⚠️ Use Chrome for mic.");return;}
  ...
  micRecognition.onresult=(e)=>{const t=[...e.results].map(r=>r[0].transcript).join("");document.getElementById("f-update").value=t;document.getElementById("char-count").textContent=t.length+" / 400";};
...
function formatDueDate(d){
  if(!d||d.trim()==="")return`<span class="lr-due due-none">—</span>`;
  const due=new Date(d+"T00:00:00");
  const now=new Date();now.setHours(0,0,0,0);
  const diff=Math.round((due-now)/(1000*60*60*24));
  const label=due.toLocaleDateString("en-IN",{day:"2-digit",month:"short",year:"numeric"});
  if(diff<0)return`<span class="lr-due due-late">${label}</span>`;
...
function confirmDeleteProject(projectName) {
  if(confirm(`Delete ALL updates for "${projectName}"?\n\nThis will permanently remove all entries for this project.`)) {
...
async function deleteProject(projectName) {
  try {
    const snap = await db.collection(COLLECTION).where("project","==",projectName).get();
    const batch = db.batch();
    snap.docs.forEach(doc => batch.delete(doc.ref));
    await batch.commit();
    if(activeProject === projectName) selectProject("all", null);
...
function confirmDelete(id, projectName) {
  if(confirm(`Delete this update for "${projectName}"?\n\nThis cannot be undone.`)) {
    deleteEntry(id);
  }
}
...
function showConfigError(){
  document.body.insertAdjacentHTML("afterbegin",`<div style="background:#fef3cd;border-bottom:2px solid #f6c90e;padding:12px 24px;text-align:center;font-size:13px;font-family:Inter,sans-serif;z-index:9999;position:relative;">⚙️ <strong>Firebase not configured.</strong> Open firebase.js and paste your config.</div>`);
  const now=Date.now();
  allDocs=[
    {id:"d1",project:"Bandra Residences",subproject:"Tower A",status:"On Track",department:"Operations",update:"Foundation complete. Moving to floor 6.",updatedBy:"Rahul Sharma",dependencies:["Material delivery"],images:[],duedate:"2026-06-15",timestamp:{toDate:()=>new Date(now-5*60*1000)}},
    ...
    {id:"d3",project:"Andheri Heights",subproject:null,status:"Completed",department:"Sales",update:"OC received. Possession June 15.",updatedBy:"Anita Desai",dependencies:[],images:[],duedate:"",timestamp:{toDate:()=>new Date(now-24*3600*1000)}},
  ];
  rebuildAll();
}
