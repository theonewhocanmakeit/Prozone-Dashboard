...
function buildListRow(c, i) {
  ...
  const dotClass = c.status==="On Track"?"dot-ontrack":c.status==="Delayed"?"dot-delayed":"dot-completed";
  const stClass  = c.status==="On Track"?"st-ontrack":c.status==="Delayed"?"st-delayed":"st-completed";
  const deptDotClass = deptClass==="operations"?"dept-dot-ops":deptClass==="finance"?"dept-dot-fin":deptClass==="hr"?"dept-dot-hr":deptClass==="sales"?"dept-dot-sales":"dept-dot-mkt";

  return `
<div class="list-row${isExp?" expanded":""}" style="animation-delay:${i*0.04}s" id="row-${c.id}">
  <div class="lr-name">
    <div class="lr-name-top"><span class="lr-icon">${icon}</span>${esc(c.project)}</div>
    ${c.subproject?`<div class="lr-name-bottom">${esc(c.subproject)}</div>`:""}
  </div>
  <div class="lr-status-col">
    <div class="status-dot-label">
      <span class="status-dot ${dotClass}"></span>
      <span class="status-text ${stClass}">${c.status}</span>
    </div>
    <div class="lr-update-text">${esc(c.update||"—")}</div>
  </div>
  <div class="lr-dept">${c.department?`<div class="dept-inline"><span class="dept-dot ${deptDotClass}"></span><span class="dept-${deptClass}">${esc(c.department)}</span></div>`:`<span style="color:var(--text-muted)">—</span>`}</div>
  <div class="lr-due">${formatDueDate(c.duedate)}</div>
  <div class="lr-dep">${deps.length?deps.map(d=>`<div class="dep-inline"><span class="dep-bullet"></span>${esc(d)}</div>`).join(""):`<span style="color:var(--text-placeholder)">—</span>`}</div>
  <div class="lr-who">
    <div class="avatar">${initials}</div>
    <div class="who-info">
      <span class="who-name">${esc(c.updatedBy||"?")}</span>
      <span class="who-time">${getTimeAgo(c.timestamp)}</span>
    </div>
  </div>
  <div class="row-actions">
    <button class="action-btn edit" onclick='openEditModal(${JSON.stringify({...c,history:undefined,images:undefined})})'>Edit</button>
