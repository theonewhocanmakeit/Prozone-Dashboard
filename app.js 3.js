...
function buildListRow(c, i) {
  ...
  return `<div class="list-row${isExp?" expanded":""}" style="animation-delay:${i*0.03}s" id="row-${c.id}">
    ...
    <div class="lr-dept">${c.department?`<span class="dept-badge dept-${(c.department||"").toLowerCase()}">${esc(c.department)}</span>`:`<span style="color:var(--text3)">—</span>`}</div>
    <div class="lr-update">${esc(c.update||"-")}</div>
    <div class="lr-due">${c.duedate?`<span class="lr-due-text">${esc(c.duedate)}</span>`:`<span style="color:var(--text3)">—</span>`}</div>
    <div class="lr-dep">${deps.length ? deps.map(d=>`<span class="dep-badge">${esc(d)}</span>`).join("") : `<span style="color:var(--text3);font-size:11px">—</span>`}</div>
...
function buildBoardCard(c) {
  ...
  return `<div class="board-card">
    ...
    ${c.department?`<div class="bc-dept"><span class="dept-badge dept-${(c.department||"").toLowerCase()}">${esc(c.department)}</span></div>`:""}
...
async function submitUpdate(e){
  ...
  try{
    await db.collection(COLLECTION).add({
      ...
      department:document.querySelector('input[name="department"]:checked')?.value||null,
...
function formatDueDate(d){
  ...
  if(diff<0)  return `<span class="lr-due due-overdue">⚠️ ${label}</span>`;
...
function toggleMic() {
  ...
  if (!("webkitSpeechRecognition" in window) && !("SpeechRecognition" in window)) {
    showToast("⚠️ Microphone not supported in this browser. Use Chrome.");
    return;
  }
  ...
  micRecognition.onresult = (e) => {
    ...
    document.getElementById("f-update").value = transcript;
    document.getElementById("char-count").textContent = transcript.length + " / 400";
  };
